Shader "Tecnocampus/PortalShader_FallbackV3"
{
    Properties
    {
        // RenderTexture of the other portal
        _MainTex ("Other Portal RT", 2D) = "black" {}
        // Mesh UV mask (alpha used as cutout)
        _MaskTex ("Mask texture", 2D) = "white" {}
        _Cutout  ("Cutout", Range(0.0, 1.0)) = 0.5

        // App-level validity flag (0/1) — prevents sampling when unlinked
        ...
        // Rotation instead of swirl
        _RotationSpeed ("Rotation Speed", Range(-20,20)) = 3.0

        // Aperture and blending
        _Open        ("Open Amount", Range(0,1)) = 0.0   // 0 closed (nothing), 1 full size
        _EdgeFeather ("Open Feather", Range(0.001,0.25)) = 0.06
        _EdgeBoost   ("Edge Boost", Range(0,5)) = 1.0
        _Link        ("Link Amount", Range(0,1)) = 0.0   // 0 show fallback, 1 show RT
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 FallbackEffect(float2 uv)
            {
                ...
                // Two scrolling layers to add motion
                ...
                // Tint + intensity
                fixed3 finalRGB = col * _FallbackColor.rgb * _FallbackIntensity;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // Aperture: radius in portal local space (assumes UV 0..1 mapped to the portal quad)
                float2 centered = i.uv * 2.0 - 1.0;
                float r = length(centered);

                // Inside factor = 1 at center when open; fades to 0 outside the aperture edge
                float inside = smoothstep(_Open, _Open - _EdgeFeather, r);

                // If completely closed, discard
                if (inside <= 0.001) clip(-1);

                // Fallback color
                fixed4 fb = FallbackEffect(i.uv);

                // Edge ring highlight when opening
                float ring = 1.0 - saturate((r - _Open) / max(_EdgeFeather * 0.5, 0.001));
                fb.rgb += _FallbackColor.rgb * ring * _EdgeBoost * 0.15;

                // Sample RT if valid
                float2 uvRT = (i.screenPos.xy / i.screenPos.w);
                fixed4 rt = tex2D(_MainTex, uvRT);
                // Null-safe: if not valid, force link weight to 0
                float linkW = saturate(_Link) * step(0.5, _LinkedPortalValid);

                // Blend between fallback and RT
                fixed4 col = lerp(fb, rt, linkW);

                // Apply aperture feather
                col.rgb *= inside;
